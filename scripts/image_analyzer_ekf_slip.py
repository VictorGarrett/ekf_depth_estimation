...
class EKFHeightDepth:
    ...
    def update(self, h_meas, z_meas, a_meas):
        H_pred, Z_pred, a_pred = self.x

        # measured vector
        z_vector = np.array([h_meas, z_meas, a_meas])
...
if __name__ == "__main__":
    ...
    H0, Z0, a0 = 1, 3.0, 0.9  # initial guesses
    P0 = np.diag([0.7**2, 2**2, 0.3**2])  # initial uncertainty
    Q = np.diag([1e-5, 8e-4, 2e-4]) # process noise 
    R = np.diag([6.0**2, 5.0**2, 0.3**2]) # measurement of height and depth noise variance 
    ...
    try:
        while not rospy.is_shutdown():
            ...
            # imu slip update
            if rospy.get_time() - last_imu_update_time > 1.0:
            ...
            if abs(image_wheel_dz) > 0.1 and latest_image is not None:
                ...
                if filtered_keypoints and latest_enc is not None:
                    ...
                    if abs(old_height-height) > 2:
                        ...
                        ekf.update_hz(height, height/(abs(old_height-height)/abs(image_wheel_dz)))
                        (H_est, Z_est, a_est), P_est = ekf.current_state()
